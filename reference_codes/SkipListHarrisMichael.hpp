...
public:
    ...
    bool contains(int key) {
        ...
        uintptr_t curr_val = pred->next[0].load();
        Node* curr = get_ptr(curr_val);
        uintptr_t succ_val = curr->next[0].load();
        return curr->key == key && !get_mark(succ_val);
    }
};
