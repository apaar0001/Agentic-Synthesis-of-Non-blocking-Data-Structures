...
public:
    ...
    bool add(int key) {
        while (true) {
            ...
            newNode->next.store(combine(w.curr, false));
            uintptr_t expected = combine(w.curr, false);
            if (w.pred->next.compare_exchange_strong(expected, combine(newNode, false))) {
                return true;
            }
            delete newNode;
        }
    ...
    bool remove(int key) {
        while (true) {
            ...
            expected = combine(w.curr, false);
            w.pred->next.compare_exchange_strong(expected, combine(succ, false));
            return true;
        }
    }

    bool contains(int key) {
        Node* curr = get_ptr(head->next.load());
        while (curr->key < key) {
            curr = get_ptr(curr->next.load());
        }
