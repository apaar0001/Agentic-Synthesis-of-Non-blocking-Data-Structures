...
CONCURRENT_DS_PATH = Path(os.path.dirname(os.path.abspath(__file__))).parent / "java_concurrent_testing" / "src" / "main" / "java" / "com" / "example" / "Sets" / "ConcurrentDataStructure.java"
SEQUENTIAL_DS_PATH = Path(os.path.dirname(os.path.abspath(__file__))).parent / "java_concurrent_testing" / "src" / "main" / "java" / "com" / "example" / "Sets" / "SequentialDataStructure.java"
BENCH_CONC_PATH = Path(os.path.dirname(os.path.abspath(__file__))).parent / "java_concurrent_testing" / "src" / "main" / "java" / "com" / "example" / "test" / "BenchMark_Concurrent.java"
LOCK_FREEDOM_PATH = Path(os.path.dirname(os.path.abspath(__file__))).parent / "java_concurrent_testing" / "src" / "main" / "java" / "com" / "example" / "test" / "LockFreedomTest.java"
_TEST_DIR = Path(os.path.dirname(os.path.abspath(__file__))).parent / "java_concurrent_testing" / "src" / "main" / "java" / "com" / "example" / "test"
BENCH_SEQ_PATH       = _TEST_DIR / "BenchmarkSequential.java"
BENCH_SEQ_QUEUE_PATH = _TEST_DIR / "BenchmarkSequentialQueue.java"
BENCH_SEQ_STACK_PATH = _TEST_DIR / "BenchmarkSequentialStack.java"

def _inject_victim_sleep(java_code: str, data_structure: str = "") -> str:
	"""
	Post-process the generated concurrent Java code to inject victim-stall logic.
	Adapts anchor comments and early-exit guards based on data structure type.
	- set/others: anchor '// Node has been marked', guards add/remove/contains
	- queue:      anchor '// Dequeue victim point',  guards enqueue/dequeue
	- stack:      anchor '// Pop victim point',      guards push/pop
	"""
	if not java_code:
		return java_code
	...
	ds = (data_structure or "").lower()
	if ds == "queue":
		anchor_comment = "// Dequeue victim point"
		guard_sigs = {
			"public void enqueue(int val)": ("void", ""),
			"public int dequeue()":         ("int",  "-1"),
		}
		stall_msg  = "Victim thread stalling inside dequeue()"
		retire_return = "return -1;"
	elif ds == "stack":
		anchor_comment = "// Pop victim point"
		guard_sigs = {
			"public void push(int val)": ("void", ""),
			"public int pop()":          ("int",  "-1"),
		}
		stall_msg  = "Victim thread stalling inside pop()"
		retire_return = "return -1;"
	else:
		anchor_comment = "// Node has been marked"
		guard_sigs = {
			"public boolean add(int key)":      ("bool", "false"),
			"public boolean remove(int key)":   ("bool", "false"),
			"public boolean contains(int key)": ("bool", "false"),
		}
		stall_msg  = "Victim thread stalling inside remove()"
		retire_return = "return true;"

	lines = java_code.splitlines()
	new_lines: list[str] = []
	inserted_helpers = False
	inserted_in_op = False
	...
	for i, line in enumerate(lines):
		...
		# Inject early-exit guard per method signature
		for sig, (ret_type, ret_val) in guard_sigs.items():
			if sig in line:
				if ret_type == "void":
					new_lines.append("        if (_lfRetired.get()) { return; }")
				else:
					new_lines.append(f"        if (_lfRetired.get()) {{ return {ret_val}; }}")
				break

		# Inject victim stall after anchor comment
		if (not inserted_in_op) and anchor_comment in line:
			new_lines.append("            // Lock-freedom victim stall injection (auto-injected)")
			new_lines.append("            if (_lfShouldStall()) {")
			new_lines.append(f"                System.err.println(\"LOG: {stall_msg}\");")
			...
			new_lines.append(f"                {retire_return}")
			new_lines.append("            }")
			inserted_in_op = True
...
def _run_bench(phase: str, timeout_seconds: int = 60, data_structure: str = "") -> subprocess.CompletedProcess:
    ...
    # Mirror user's manual command for both phases: no exec.args; rely on defaults in benchmark classes
    exec_args = None
    for mvn_path in mvn_paths:
        try:
            cmd = [
...
def _restore_extra_bench(extra_bench_pairs: list, log_file: str) -> None:
	"""Restore any bench files that were temporarily hidden."""
	for orig, bak in extra_bench_pairs:
		if bak.exists():
			try:
				bak.rename(orig)
				_write_to_logs(log_file, f"[RESTORE] Restored {orig.name}
")
			except Exception:
				pass
...
def _run_test_logic(state: GraphState, phase: str) -> Dict[str, Any]:
	def _with_history(result: Dict[str, Any]) -> Dict[str, Any]:
		...
		# Use the attempt number from the latest node or calculate it
		attempt_no = state.get("conc_attempt_count", 0)
		if attempt_no == 0:
			attempt_no = len(history) + 1
		
		# Look for existing entry for this attempt (e.g., added by verify_structural)
		existing_entry = None
		for entry in history:
			if entry.get("attempt") == attempt_no:
		...
		new_data = {
			...
			"structural_verify_status": result.get("structural_verify_status", state.get("structural_verify_status", "none")),
	...
	try:
		...
		ds = (state.get("data_structure") or "").lower()
		_section(log_file, f"[{phase.upper()}] TEST START")
		_write_to_logs(log_file, f"[{phase.upper()}] Starting COMPILE...\n")

		# For sequential phase, temporarily hide concurrent-related sources to avoid compile of broken conc files
		conc_backup = CONCURRENT_DS_PATH.with_suffix(".java.bak")
		bench_conc_backup = BENCH_CONC_PATH.with_suffix(".java.bak")
		lf_backup = LOCK_FREEDOM_PATH.with_suffix(".java.bak")
		# Also hide conflicting DS-specific sequential benchmarks
		bench_seq_backup       = BENCH_SEQ_PATH.with_suffix(".java.bak")
		bench_seq_queue_backup = BENCH_SEQ_QUEUE_PATH.with_suffix(".java.bak")
		bench_seq_stack_backup = BENCH_SEQ_STACK_PATH.with_suffix(".java.bak")
		# Determine which seq benchmark files conflict with current DS (applies to both seq and conc phases)
		if ds == "queue":
			extra_bench_pairs = [(BENCH_SEQ_PATH, bench_seq_backup), (BENCH_SEQ_STACK_PATH, bench_seq_stack_backup)]
		elif ds == "stack":
			extra_bench_pairs = [(BENCH_SEQ_PATH, bench_seq_backup), (BENCH_SEQ_QUEUE_PATH, bench_seq_queue_backup)]
		else:
			extra_bench_pairs = [(BENCH_SEQ_QUEUE_PATH, bench_seq_queue_backup), (BENCH_SEQ_STACK_PATH, bench_seq_stack_backup)]

		conc_moved = False
		if phase == "seq":
			# In seq phase: also hide concurrent DS file and lock-freedom test
			for src, dst in [
				(CONCURRENT_DS_PATH, conc_backup),
				(BENCH_CONC_PATH, bench_conc_backup),
				(LOCK_FREEDOM_PATH, lf_backup),
			] + [(dst, src) for src, dst in extra_bench_pairs]:
				if src.exists():
					try:
						src.rename(dst)
						conc_moved = True
						_write_to_logs(log_file, f"[SEQ] Temporarily moved {src.name} to {dst}\n")
					except Exception as e:
						_write_to_logs(log_file, f"[SEQ] WARNING: Failed to move {src.name}: {e}\n")
		elif phase == "conc":
			# In conc phase: only hide conflicting seq benchmark files
			for src, dst in extra_bench_pairs:
				if src.exists():
					try:
						src.rename(dst)
						conc_moved = True
				...
				if conc_moved:
					for src, dst in [
						...
						if src.exists():
							try:
								src.rename(dst)
								_write_to_logs(log_file, f"[SEQ] Restored {dst.name} after compile\n")
							except Exception as e:
		...
		if phase == "conc":
			...
			b1 = _run_bench("conc", timeout_seconds=60, data_structure=ds)
			...
			if b1.returncode != 0:
				...
				_write_to_logs(log_file, message)
				_restore_extra_bench(extra_bench_pairs, log_file)
				return _with_history({
					"test_result": "fail",
					"error_message": out1,
			...
			instrumented_code = _inject_victim_sleep(original_code, data_structure=ds)
			try:
			...
			b2 = _run_bench("conc_victim", timeout_seconds=60, data_structure=ds)
		...
		b = _run_bench(phase, timeout_seconds=60, data_structure=ds)
		if b.returncode != 0:
		...
		if conc_moved:
			for src, dst in [
				...
				if src.exists():
					try:
						src.rename(dst)
						_write_to_logs(log_file, f"[SEQ] Restored {dst.name} after tests\n")
					except Exception as e:
						_write_to_logs(log_file, f"[SEQ] WARNING: Failed to restore {dst.name}: {e}\n")
