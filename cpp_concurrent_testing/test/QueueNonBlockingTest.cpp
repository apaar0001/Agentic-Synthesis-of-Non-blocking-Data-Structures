...
/**
 * Queue Non-Blocking (Lock-Freedom) Test — mirrors Java QueueNonBlockingTest.
 *
 * The ConcurrentDataStructure is pre-instrumented by the Python victim_injector
 * with sleep(5s) after successful CAS in dequeue().
...
int main(int argc, char** argv) {
    ...
    std::cout << "Queue NonBlocking Experiment: Algo:" << config.algo
    ...
    for (int i = 0; i < config.numThreads; i++) {
        threads.emplace_back(RunOperationsQueue::benchMarkRun, queue.get(), i,
    ...
    if (passed) {
        ...
        std::cout << "[QueueNonBlockingTest] Progress check failed: opsBefore="
