...
/**
 * Consistency Test (Sanity Test 1) for concurrent data structures.
 *
 * Runs a linearizability/consistency check on the ConcurrentDataStructure
 * using multiple threads. Verifies that the aggregate add/remove counts per
 * key match the expected state as observed by contains().
 *
 * Uses RunOperations + RunController (the standard deterministic harness).
 */
public class ConsistencyTest {
    ...
    private static void warmupVM() {
        ...
        try {
            ...
            for (int i = 0; i < threads.length; i++) {
                threads[i] = new Thread(new RunOperations(set, i, addPercent, removePercent, keyRange, results,
    ...
    private static void helpUser() {
        String help = "Concurrent Set Consistency Test\n" +
                "\nUsage:\n  ConsistencyTest [options...]\n" +
                "\nOptions:\n" +
                "  -h, --help\n        Print this message\n" +
    ...
    /**
     * Main entry point for the consistency test.
     *
    ...
    public static void main(String[] args) {
        try {
            ...
            if (testSanity) {
                ...
                // Reset the set for the main benchmark phase.
                set = null;
                defineSet();
                Tools.cleanMem(memTree);
                memTree = Tools.getMemUsed();
            ...
            System.err.println("[ConsistencyTest] FATAL: Unhandled exception from generated ConcurrentDataStructure:");
