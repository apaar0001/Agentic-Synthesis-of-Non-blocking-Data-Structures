                ...
                while (true) {
                    ...
                    while (is_marked_ref(succ)) {
                        ...
                        if (!pred->forward[i].compare_exchange_strong(
                            curr, unmarked_succ, std::memory_order_acq_rel, std::memory_order_acquire)) {
                            retry = true;
                            break;
                        }
...
public:
    ...
    bool contains(int key) override {
        ...
        return get_unmarked_ref(pred->forward[0].load(std::memory_order_acquire))->val == key;
    }
        ...
        while (true) {
            ...
            for (int i = 1; i <= newLevel; ++i) {
                while (true) {
                    ...
                    if (succs[0]->val != key) {
                        for (int j = i; j <= newLevel; ++j) {
                            newNode->forward[j].store(nullptr, std::memory_order_relaxed);
                        }
                        delete newNode;
                        return false;
                    }
        ...
        while (true) {
            ...
            for (int i = nodeToRemove->topLevel; i >= 1; --i) {
                while (true) {
                    ...
                    if (nodeToRemove->forward[i].compare_exchange_strong(
