...
public:
    ...
    ~ConcurrentDataStructure() {
        // Not implemented for strict lock-freedom correctness
    }
    ...
    bool remove(int key) override {
        ...
        for (int i = 0; i < MAX_LEVEL; ++i) {
            while (true) {
                ...
                Node* newNext = next->forward[i].load(std::memory_order_acquire);
                if (!update[i]->forward[i].compare_exchange_strong(next, newNext, std::memory_order_acq_rel)) {
                    continue;
                }
                break;
            }
