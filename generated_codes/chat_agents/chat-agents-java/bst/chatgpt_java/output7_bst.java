...
public boolean add(int key) {
    while (true) {
        ...
        if (observed == curr && !link.isMarked() && !parent.isMarked()) {
...
private Window find(int key) {
    ...
    while (true) {
        ...
        while (curr != null) {
            if (curr.isMarked()) {
                boolean unlinked = helpUnlink(parent, curr);
                if (!unlinked && hasAtMostOneChild(curr)) {
                    continue retry;
                }
                ...
                if (parent == null) {
                    ...
                    curr = key < parent.key ? parent.left.getReference() : parent.right.getReference();
                }
                continue;
            }
