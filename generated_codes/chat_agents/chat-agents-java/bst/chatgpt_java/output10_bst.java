...
public boolean add(int key) {
    while (true) {
        ...
        AtomicMarkableReference<Node> child = key < parent.key ? parent.left : parent.right;
        Node observed = child.getReference();

        if (observed == curr && !child.isMarked() && !parent.marked()) {
            if (child.compareAndSet(curr, new Node(key), false, false)) {
                return true;
            }
        }
    }
