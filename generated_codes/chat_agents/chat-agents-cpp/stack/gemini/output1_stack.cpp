...
public:
    ...
    int pop() override {
        ...
        while (true) {
            ...
            if (unmarked_top->next.compare_exchange_strong(next_node, marked_next, std::memory_order_acq_rel, std::memory_order_acquire)) {
                ...
                top.compare_exchange_strong(expected_top, get_unmarked_ref(next_node), std::memory_order_acq_rel, std::memory_order_relaxed);
                return result;
            }
    ...
    bool isEmpty() override {
        ...
        while (curr_top != nullptr) {
            ...
            if (is_marked_ref(next_node)) {
                Node* unmarked_next = get_unmarked_ref(next_node);
                top.compare_exchange_strong(curr_top, unmarked_next, std::memory_order_acq_rel, std::memory_order_acquire);
                curr_top = top.load(std::memory_order_acquire);
            } else {
                return false;
            }
