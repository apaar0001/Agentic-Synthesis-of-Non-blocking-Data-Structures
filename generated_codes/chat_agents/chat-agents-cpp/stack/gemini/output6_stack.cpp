...
public:
    ...
    void push(int val) override {
        ...
        while (true) {
            ...
            if (top.compare_exchange_weak(curr_top, new_node, std::memory_order_acq_rel, std::memory_order_acquire)) {
                return;
            }
        }
    }
    ...
    int pop() override {
        ...
        while (true) {
            ...
            if (is_marked_ref(next_node)) {
                ...
                if (top.compare_exchange_weak(curr_top, unmarked_next, std::memory_order_acq_rel, std::memory_order_acquire)) {
                    continue;
                }
            ...
            if (unmarked_top != get_unmarked_ref(top.load(std::memory_order_relaxed))) {
                curr_top = top.load(std::memory_order_acquire);
                continue;
            }
            ...
            if (unmarked_top->next.compare_exchange_strong(next_node, marked_next, std::memory_order_acq_rel, std::memory_order_acquire)) {
                ...
                Node* desired_top = get_unmarked_ref(next_node);
                if (top.compare_exchange_strong(curr_top, desired_top, std::memory_order_acq_rel, std::memory_order_acquire)) {
                    // Deferred reclamation should be handled in a production system.
                    // To guarantee safety, we leave deletion to an epoch-based reclaimer.
                }
                return result;
            }
            curr_top = top.load(std::memory_order_acquire);
