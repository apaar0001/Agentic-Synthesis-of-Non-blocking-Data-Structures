...
public:
    ...
    int pop() override {
        ...
        while (true) {
            ...
            if (unmarked_top->next.compare_exchange_strong(next_node, marked_next, std::memory_order_acq_rel, std::memory_order_acquire)) {
                ...
                top.compare_exchange_strong(expected_top, get_unmarked_ref(next_node), std::memory_order_acq_rel, std::memory_order_acquire);
                return result;
            }
    ...
    bool isEmpty() override {
        Node* curr_top = top.load(std::memory_order_acquire);
        return get_unmarked_ref(curr_top) == nullptr;
    }
};
