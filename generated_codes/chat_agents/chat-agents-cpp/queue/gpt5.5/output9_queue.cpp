...
public:
    ...
    void enqueue(int val) override {
        ...
        while (true) {
            Node* last = get_unmarked_ref(tail.load(std::memory_order_acquire));
            Node* next = last->next.load(std::memory_order_acquire);

            if (last != get_unmarked_ref(tail.load(std::memory_order_acquire))) {
                continue;
            }
    ...
    int dequeue() override {
        while (true) {
            Node* first = get_unmarked_ref(head.load(std::memory_order_acquire));
            Node* last = get_unmarked_ref(tail.load(std::memory_order_acquire));
            Node* next = first->next.load(std::memory_order_acquire);

            if (first != get_unmarked_ref(head.load(std::memory_order_acquire))) {
                continue;
            }
            ...
            Node* observedNext = next;
            Node* markedNext = get_marked_ref(unmarkedNext);

            if (!is_marked_ref(observedNext)) {
            ...
            if (head.compare_exchange_weak(
                ...
                first->next.store(
                    get_marked_ref(get_unmarked_ref(first->next.load(std::memory_order_relaxed))),
