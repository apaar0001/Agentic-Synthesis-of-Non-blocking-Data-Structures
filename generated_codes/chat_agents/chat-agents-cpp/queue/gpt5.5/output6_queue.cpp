...
private:
    ...
    void help_remove_marked(Node* prev, Node* curr) {
    ...
    int dequeue() override {
        while (true) {
            ...
            if (first == head.load(std::memory_order_acquire)) {
                ...
                if (first_unmarked == last_unmarked) {
                    tail.compare_exchange_strong(
                        last,
                        next_unmarked,
                        std::memory_order_acq_rel,
                ...
                if (!is_marked_ref(next_next)) {
                    ...
                    next_unmarked->next.compare_exchange_strong(
                        next_next,
                        marked_next,
                        std::memory_order_acq_rel,
                        std::memory_order_acquire
    ...
    bool isEmpty() override {
        while (true) {
            ...
            if (first == head.load(std::memory_order_acquire)) {
                ...
                if (is_marked_ref(next_next)) {
                    Node* expected = next_unmarked;

                    first_unmarked->next.compare_exchange_strong(
                        expected,
                        get_unmarked_ref(next_next),
                        std::memory_order_acq_rel,
                        std::memory_order_acquire
                    );
