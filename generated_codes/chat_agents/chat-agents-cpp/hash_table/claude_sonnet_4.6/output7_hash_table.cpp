...
private:
    ...
    retry:
        ...
        while (true) {
            ...
            if (marked) {
                Node* exp = uc;
                if (!pred->compare_exchange_weak(exp, succ,
...
public:
    ...
    bool contains(int key) override {
        ...
        while (cur) {
            if (cur->val == key &&
                !is_marked_ref(cur->next.load(std::memory_order_acquire)))
                return true;
    ...
    bool add(int key) override {
        ...
        while (true) {
            ...
            Node* exp = curr;
            if (pred->compare_exchange_weak(exp, node,
                    std::memory_order_acq_rel,
                    std::memory_order_acquire))
                return true;
            delete node;
    ...
    bool remove(int key) override {
        ...
        while (true) {
            ...
            if (!curr->next.compare_exchange_weak(succ, marked,
                    std::memory_order_acq_rel,
                    std::memory_order_acquire))
                continue;
            Node* exp = curr;
            pred->compare_exchange_weak(exp, succ,
                std::memory_order_acq_rel,
                std::memory_order_acquire);
            return true;
        }
