...
class ConcurrentDataStructure : public SetADT {
    ...
    // Every traversal helps physically unlink any marked node it encounters
    void help_remove(Node* pred, Node* marked_curr, Node* succ) {
        pred->next.compare_exchange_strong(marked_curr, succ,
...
public:
    ...
    bool contains(int key) override {
        ...
        Node* raw = curr->next.load(std::memory_order_acquire);
        return curr->val == key && !is_marked_ref(raw);
    }
    ...
    bool remove(int key) override {
        while (true) {
            ...
            // Logical mark — other threads will help finish the physical unlink
