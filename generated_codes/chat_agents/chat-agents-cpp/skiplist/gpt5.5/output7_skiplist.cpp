...
class ConcurrentDataStructure : public SetADT {
    ...
    struct Node {
        ...
        Node(int v, int level) : val(v), topLevel(level) {
            for (int i = 0; i < MAX_LEVEL; ++i) forward[i].store(nullptr, std::memory_order_relaxed);
        }
    };
    ...
    int randomLevel() {
        ...
        int level = 0;
        while (level < MAX_LEVEL - 1 && (gen() & 1)) ++level;
        return level;
    }
    ...
    retry:
        ...
        for (int level = MAX_LEVEL - 1; level >= 0; --level) {
            ...
            while (true) {
                Node* succ = curr ? curr->forward[level].load(std::memory_order_acquire) : nullptr;
                while (curr && is_marked_ref(succ)) {
                    Node* unmarkedSucc = get_unmarked_ref(succ);
                    Node* expected = curr;
                    if (!pred->forward[level].compare_exchange_strong(
                            expected, unmarkedSucc,
                            std::memory_order_acq_rel,
                            std::memory_order_acquire)) {
                        goto retry;
                    }
                    curr = get_unmarked_ref(pred->forward[level].load(std::memory_order_acquire));
                    succ = curr ? curr->forward[level].load(std::memory_order_acquire) : nullptr;
                }
        ...
        return succs[0] && succs[0]->val == key;
    }
    ...
    bool add(int key) override {
        ...
        while (true) {
            ...
            for (int level = 0; level <= topLevel; ++level) {
                node->forward[level].store(succs[level], std::memory_order_relaxed);
            }
